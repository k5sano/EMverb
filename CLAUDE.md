# CloudsReverb — Claude Code プロジェクト設定

## プロジェクト概要

Mutable Instruments Clouds の内蔵リバーブ (Dattorro plate reverb) を抽出した VST3 リバーブプラグイン。JUCE フレームワークで開発。 外部依存 (eurorack / stmlib) なしで自己完結する。
