#include "SampleRateAdapter.h" #include "DattorroReverb.h" #include #include
...
void SampleRateAdapter::process(const float\* inL, const float\* inR, float\* outL, float\* outR, int numSamples, DattorroReverb& reverb) { if (numSamples <= 0) return;

```
// If host rate ≈ 32kHz, pass through directly
if (std::abs(hostSampleRate_ - kInternalSampleRate) < 1.0)
{
    // Copy input to engine buffers, process in-place
    for (int i = 0; i < numSamples; ++i)
    {
        outL[i] = inL[i];
        outR[i] = inR[i];
    }
    reverb.process(outL, outR, numSamples);
    return;
}
...
// Step 2: Downsample → reverb → output ring
